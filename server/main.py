...
class PDFUpload(BaseModel):
    title: str
    file_url: str
    uploaded_by: str  # user's email or id
...
async def oauth2callback(request: Request):
    ...
    return {"message": "Authentication successful!", "credentials": credentials_json}


@app.post("/upload")
async def upload_file(file: UploadFile = File(...), background_tasks: BackgroundTasks = None):
    # uploads PDF to Google Drive, stores URL in PostgreSQL
    credentials_json = "user_credentials_json" # gets credentials
    ...
    background_tasks.add_task(store_file_metadata, file.filename, file_url, user_email)

    return {"message": "File uploaded successfully!", "file_url": file_url}


async def store_file_metadata(title: str, file_url: str, uploader: str):
    # stores uploaded file metadata in PostgreSQL
    await db.connect()
    await db.fileupload.create(data={"title": title, "file_url": file_url, "uploader": uploader})
    await db.disconnect()